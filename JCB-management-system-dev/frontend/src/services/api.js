...
const api = axios.create({
  ...
  withCredentials: true, // Add this to ensure cookies/credentials are sent
});
...
export const jcbAPI = {
  ...
  updateJCB: async (registeredNumber, jcbData) => {
    const response = await api.put(`/api/jcbs/${registeredNumber}`, jcbData);
    return response.data;
  },
  
  deleteJCB: async (registeredNumber) => {
    const response = await api.delete(`/api/jcbs/${registeredNumber}`);
    return response.data;
  }
};
